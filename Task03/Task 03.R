...
setwd("~/Desktop/Evolution/Tasks/Task03")
library(paleobioDB)
Taxon <- "Dinosauria"
MinMA <- 66
MaxMA <- 252
...
# These 2 lines seem to rise similarly, but I am surprised by the large spike in "first appear" line after ~100 million yrs ago. Assuming these numbers are accurate, dinosaurs were evolving the fastest ~50-60 million yrs ago.
...
lines(seq(to=MaxMA, from=MinMA, length.out=nrow(nspeciesOverTime4)), nspeciesOverTime4[,2], col=Col_reptilia, lwd=LineWidth)
legend("topleft", legend=c(Taxon3, Taxon4), col=c(Col_echino, Col_reptilia), bty="n", lwd=LineWidth)
# I hypothesize that the changing number of species of Echinodermata and Reptilia are positively correlated to one another. 
